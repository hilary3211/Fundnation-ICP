...
module {
  ...
  public func send(network : Network, derivation_path : [[Nat8]], key_name : Text, dst_address : BitcoinAddress, amount : Satoshi) : async [Nat8] {
    ...
    let transaction =
        Utils.get_ok2(Transaction.fromBytes(Iter.fromArray(tx_bytes)));
    ...
    let signed_transaction = Utils.get_ok2(Transaction.fromBytes(Iter.fromArray(signed_transaction_bytes)));
...
public func build_transaction(
    ...
    loop {
        let transaction =
            Utils.get_ok_except2(Bitcoin.buildTransaction(2, own_utxos, [(#p2pkh dst_address, amount)], #p2pkh own_address, Nat64.fromNat(total_fee)), "Error building transaction.");
  ...
  func public_key_bytes_to_public_key(public_key_bytes : [Nat8]) : PublicKey {
      ...
      Utils.get_ok2(Publickey.decode(#point point))
  };
}